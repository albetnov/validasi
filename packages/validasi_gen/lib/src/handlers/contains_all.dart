...
class ContainsAllGen extends RuleGen {
  @override
  String get name => 'ContainsAll';
  ...
  RuleInfo parse(ConstantReader rule) {
    final elements = rule
        .read('elements')
        .listValue
        .map((e) => _literal(ConstantReader(e)))
        .toList();
    return RuleInfo('ContainsAll', {'elements': elements},
  ...
  String defaultMessage(RuleInfo info,
      ...
      'List must contain all required elements';
  ...
  String emitError(RuleInfo info, String pathExpr, String messageArg,
    ...
    final elements = info.params['elements'] as List<String>;
    final elementsLiteral = '[${elements.join(', ')}]';
    return '_Errors.containsAll($pathExpr, $elementsLiteral$messageArg)';
  }

  @override
  Map<String, String> get helperMethods => {
        'containsAll':
            "static ValidationError containsAll(List<String> path, List<Object?> elements, {String? message}) =>\n"
                '      ValidationError(\n'
                "        rule: 'ContainsAll',\n"
