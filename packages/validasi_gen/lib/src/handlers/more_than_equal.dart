...
class MoreThanEqualGen extends RuleGen {
  @override
  String get name => 'MoreThanEqual';
  ...
  RuleInfo parse(ConstantReader rule) {
    ...
    return RuleInfo(
        'MoreThanEqual', {'min': min}, rule.peek('message')?.stringValue);
  }
  ...
  String defaultMessage(RuleInfo info,
      ...
      'Value must be more than or equal to ${info.params['min']}';

  @override
  String emitError(RuleInfo info, String pathExpr, String messageArg,
          [FieldContext context = FieldContext.generic]) =>
      '_Errors.moreThanEqual($pathExpr, ${info.params['min']}$messageArg)';

  @override
  Map<String, String> get helperMethods => {
        'moreThanEqual':
            "static ValidationError moreThanEqual(List<String> path, num min, {String? message}) =>\n"
                '      ValidationError(\n'
                "        rule: 'MoreThanEqual',\n"
